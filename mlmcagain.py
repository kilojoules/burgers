...
lfdat2 = pd.read_csv('./morelfsamps.dat')
lfdat2.rename(columns={'rnx7000':'lf1', 'nx400':'lf2', 'nu':'visc', 'trnx7000': 'tlf1', 'tnx400': 'tlf2'}, inplace=1)
mylfdat = pd.concat([lfdat, lfdat2])
...
e = np.linspace(0, 500, 1000)
r = np.exp(np.linspace(-10, 1, 100))
X, Y = np.meshgrid(e, r)
plt.contour(X, Y, np.sqrt(X) * Y, 300, colors='k')
plt.plot(x, np.mean(np.abs(TRUTH - hfest), 1), label=r'$Q^{HF}$', marker='x')
plt.plot(x, np.mean(np.abs(TRUTH - lfest), 1), label=r'$Q^{{Reduced}}$', marker='x')
plt.plot(xcv, np.mean(np.abs(TRUTH - cvest), 1), label=r'$Q^{CV}$', marker='x')
#plt.plot(xcv2, np.mean(np.abs(TRUTH - cv2est), 1), label='CV2', marker='x')
plt.legend(ncol=3, bbox_to_anchor=[.3, 1.3], prop={'size':16})
plt.yscale('log')
plt.xscale('log')
plt.ylabel(r'$|\mathbb{E}[f_0(\nu)] - Q|$')
plt.xlabel('High-Fidelity Samples')
plt.xlim(np.min(xcv), np.max(x))
plt.ylim(np.min(np.mean(np.abs(TRUTH - cvest), 1)), np.max(np.mean(np.abs(TRUTH - cvest), 1)))
plt.savefig('convPlot.pdf', bbox_inches='tight')
plt.clf()

e = np.linspace(0, 100000, 1000)
r = np.exp(np.linspace(-10, 1, 100))
X, Y = np.meshgrid(e, r)
plt.contour(X, Y, np.sqrt(X) * Y, 300, colors='k')
plt.plot(hft.mean(1), np.mean(np.abs(TRUTH - hfest), 1), label=r'$Q^{HF}$', marker='x')
plt.plot(lft.mean(1), np.mean(np.abs(TRUTH - lfest), 1), label=r'$Q^{Reduced}$', marker='x')
plt.plot(cvt.mean(1), np.mean(np.abs(TRUTH - cvest), 1), label=r'$Q^{CV}$', marker='x')
...
plt.xlim(np.min(cvt.mean(1)), np.max(hft.mean(1)))
plt.ylim(np.min(np.mean(np.abs(TRUTH - cvest), 1)), np.max(np.mean(np.abs(TRUTH - hfest), 1)))
...
ax[1].set_xlabel(r'$Q^{\mathrm{Reduced}}$')
...
plt.plot(hfsamps[1:], hfDs, label=r'$Q^{\mathrm{HF}}$')
plt.plot(SAMPS[1:], lfDs, label=r'$Q^{\mathrm{Reduced}}$')
plt.plot(SAMPS[1:], cvDs, label=r'$Q^{\mathrm{CV}}$')
plt.ylabel('D')
...
ax.set_xscale('log')
ax.set_yscale('log')
#ax.set_xticks([1e3, 5e3])
#ax.set_xticklabels([1e3, 5e3])
#for ii, a in enumerate(ax.get_xticklabels()):
#    if ii % 2 == 1: a.set_visible(False)
...
for ii in [np.where(x==2)[0][0], np.where(x==152)[0][0], np.where(x==xcv[-1])[0][0]]:
  for jj in range(hfest.shape[1]):
    viohf.append(hfest[ii, jj])
    viorf.append(lfest[ii, jj])
    viox.append(x[ii])
for ii in [np.where(xcv==2)[0][0], np.where(xcv==152)[0][0], np.where(xcv==xcv[-1])[0][0]]:
...
ax[1].set_ylabel(r'$Q^{Reduced}$')
ax[2].set_ylabel(r'$Q^{CV}$')
